...
\begin{document}
	...
	\section{Wstęp}
		Celem sprawozdania jest weryfikacja metod testowania hipotez statystycznych dotyczących średniej i wariancji. W pracy przeprowadzono symulacje komputerowe, których zadaniem było wyznaczenie wartości statystyk testowych, obszarów krytycznych oraz p-wartości. Analiza obejmuje ocenę skuteczności testów poprzez badanie błędów I i II rodzaju oraz wyznaczenie mocy testów w warunkach dużej liczebności próby.
	...
	Wykres pudełkowy przedstawia porównanie empirycznych wartości błędu I rodzaju z ich wartościami teoretycznymi dla trzech poziomów istotności ($\alpha = 0.01; 0.05; 0.10$). Badanie przeprowadzone dla stu niezależnych powtórzeń pozwala ocenić poprawność i stabilność testu w zależności od wybranego poziomu istotności oraz rodzaju hipotezy alternatywnej. \\
	
	Wykres podzielono na trzy panele odpowiadające kolejnym wartościom $\alpha$, z których każdy zawiera trzy boxploty obrazujące rozkłady wyników. Główna część wykresu, czyli pudełko, wyznacza rozstęp międzykwartylowy skupiający połowę uzyskanych wartości empirycznych. Przebiegająca przez środek pomarańczowa linia to mediana, wyznaczająca punkt, wokół którego oscylowały wyniki symulacji. Pionowe wąsy pokazują zakres większości pozostałych rezultatów, pojedyncze punkty poza nimi to rzadkie wartości odstające, natomiast poziome, różowe linie odzwierciedlają idealne wartości teoretyczne. \\
	
	Z analizy wykresów płyną istotne wnioski statystyczne. Przede wszystkim empiryczne mediany we wszystkich dziewięciu przypadkach niemal idealnie pokrywają się z liniami teoretycznymi, co dowodzi, że algorytm testujący działa prawidłowo i ściśle zachowuje zadany reżim istotności. Ponadto zbliżona, wąska szerokość pudełek i wąsów niezależnie od kierunku hipotezy alternatywnej świadczy o wysokiej stabilności numerycznej procedury. Delikatny wzrost rozrzutu wyników wraz ze wzrostem $\alpha$ jest naturalną konsekwencją szerszych obszarów krytycznych. Podsumowując, eksperyment potwierdza, że rzeczywiste prawdopodobieństwo popełnienia błędu I rodzaju odpowiada założonemu poziomowi istotności $\alpha$, co czyni test w pełni wiarygodnym narzędziem. \\
		\newpage
...
\end{figure}
	...
	\section{Wnioski}
	Symulacje potwierdziły teoretyczną poprawność testów statystycznych – empiryczny błąd I rodzaju jest zgodny z założonym poziomem $\alpha$. Analiza wykazała, że moc testu rośnie wraz z dystansem między parametrem rzeczywistym a hipotezowanym. 
